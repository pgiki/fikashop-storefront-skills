...
| Collections | Optional shelves | Catalog / collections UI | `GET /ranges/`, `GET /products/?range=` | [curl/list-ranges.sh](examples/curl/list-ranges.sh), [CATALOG.md](../contracts/CATALOG.md#product-groups-ranges) |
...
| Cart / promo | Cart / checkout | `BasketStore`, checkout UI | `POST /basket/add-voucher/` then `GET /basket/` | [curl/add-voucher.sh](examples/curl/add-voucher.sh), [INTEGRATION.md](../contracts/INTEGRATION.md#voucher-promo-code) |
